...
const Validation = {
  configuration(CONFIG) {
    ...
    // KEYS
    if (!process.env.BINANCE_API && CONFIG.EXECUTION.ENABLED) {
      const msg = `Trade executions will fail without an api key (KEY.API)`;
      logger.execution.warn(msg);
    }
    if (!process.env.BINANCE_SECRET === "" && CONFIG.EXECUTION.ENABLED) {
    ...
    Object.keys(CONFIG.INVESTMENT).forEach((BASE) => {
      ...
      if (
        isNaN(CONFIG.INVESTMENT[BASE].MIN) ||
        CONFIG.INVESTMENT[BASE].MIN <= 0
      ) {
      ...
      if (
        isNaN(CONFIG.INVESTMENT[BASE].MAX) ||
        CONFIG.INVESTMENT[BASE].MAX <= 0
      ) {
      ...
      if (
        isNaN(CONFIG.INVESTMENT[BASE].STEP) ||
        CONFIG.INVESTMENT[BASE].STEP <= 0
      ) {
      ...
      if (
        CONFIG.INVESTMENT[BASE].MIN !== CONFIG.INVESTMENT[BASE].MAX &&
        CONFIG.INVESTMENT[BASE].MIN + CONFIG.INVESTMENT[BASE].STEP >
          CONFIG.INVESTMENT[BASE].MAX
      ) {
    ...
    // SCANNING
    if (
      !Number.isInteger(CONFIG.SCANNING.DEPTH) ||
      CONFIG.SCANNING.DEPTH <= 0
    ) {
    ...
    if (
      ...
      CONFIG.SCANNING.WHITELIST.length === 0
    ) {
    ...
    if (
      !CONFIG.EXECUTION.TEMPLATE.every((template) =>
    ...
    if (
      !Number.isInteger(CONFIG.EXECUTION.THRESHOLD.AGE) ||
      CONFIG.EXECUTION.THRESHOLD.AGE <= 0
    ) {
    ...
    if (
      !Number.isInteger(CONFIG.HUD.REFRESH_RATE) ||
      CONFIG.HUD.REFRESH_RATE <= 0
    ) {
    ...
    if (
      isNaN(CONFIG.LOG.STATUS_UPDATE_INTERVAL) ||
      CONFIG.LOG.STATUS_UPDATE_INTERVAL < 0
    ) {
    ...
    // WEBSOCKET
    if (
      !Number.isInteger(CONFIG.WEBSOCKET.BUNDLE_SIZE) ||
      CONFIG.WEBSOCKET.BUNDLE_SIZE <= 0
    ) {
    ...
    if (
      !Number.isInteger(CONFIG.WEBSOCKET.INITIALIZATION_INTERVAL) ||
      CONFIG.WEBSOCKET.INITIALIZATION_INTERVAL < 0
    ) {
