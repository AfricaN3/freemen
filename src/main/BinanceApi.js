...
const binance = new Binance().options(
  Object.assign(
    {
      APIKEY: CONFIG?.KEYS?.API || process.env.BINANCE_API,
      APISECRET: CONFIG?.KEYS?.SECRET || process.env.BINANCE_SECRET,
...
const BinanceApi = {
  ...
  getDepthSnapshots(tickers, maxDepth = CONFIG.SCANNING.DEPTH) {
    ...
    tickers.forEach((ticker) => {
      if (
        BinanceApi.sortedDepthCache[ticker].eventTime ===
        binance.depthCache(ticker).eventTime
      ) {
        depthSnapshot[ticker] = { ...BinanceApi.sortedDepthCache[ticker] };
      } else {
        depthSnapshot[ticker] = { ...BinanceApi.sortedDepthCache[ticker] } = {
          ...BinanceApi.getDepthCacheSorted(ticker, maxDepth),
        };
      }
    });
  ...
  marketBuy(ticker, quantity) {
    ...
    return binance
      .marketBuy(ticker, quantity, { type: "MARKET" })
      .then((response) => {
        if (binance.getOption("test")) {
          ...
          logger.execution.info(
            ...
            } in ${Util.millisecondsSince(before)} ms`
          );
        }
        return response;
      })
  ...
  marketSell(ticker, quantity) {
    ...
    const before = Date.now();
    return binance
      .marketSell(ticker, quantity, { type: "MARKET" })
      .then((response) => {
        if (binance.getOption("test")) {
          logger.execution.info(
  ...
  depthCacheStaggered(tickers, limit, stagger, cb) {
    tickers.forEach(
      (ticker) => (BinanceApi.sortedDepthCache[ticker] = { eventTime: 0 })
    );
    return binance.websockets.depthCacheStaggered(
      tickers,
      BinanceApi.createDepthWSCallback(cb),
      limit,
      stagger
    );
  },
